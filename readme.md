 ...
 * Prefer single quotes simply because they don’t require pressing shift
 * Look to existing, popular libraries for ideas on architectural boundaries that actually work
   * semantic-ui for abstract component props
 * Functions follow {verb}[{noun}] format
 * Just make the whole database schema first
 * Start a new codebase and database for a new project if at all possible. Having to account for existing code or data or schemas slows the team down.
 * Clearly denote hacks; make them awkwardly obvious with variable names and comments
 * The most likely cause of the bug is you, not the tools
 * Prefer map/filter/reduce to for loops
 * Do one thing at a time; handle one domain at a time
 * Create clear sections within a file or function with over-the-top comments
 ...
 * Use a setup function instead of beforeEach, if ever necessary (KISS)
 * Leave the database messy after testing (perform clean up in set up) so you can observe values in the case of a failure
 * Divide tests into four sections: set up, test code, assertions, clean up
 ...
 * Frameworks/IoC/training-wheels usually suck
 * Writing a few extra lines of code on the concrete level to gain the ultimate abstraction is worth it
 ...
 * Handle errors first, then do what the function says it does
 ...
 * If you need speed, create computed tables. CSV/filesystem is too clunky and rigid. Materialized views take too long to compute, can’t be parallelized/made more efficient, don’t have progress bars, are difficult to edit
 * Create scrape_data, cache_data, load_data, train, predict, and get_predictions functions
 * Make everything “lean” (aka, one-at-a-time), even when caching data. The simplicity and robustness trumps potential compute-time efficiency
